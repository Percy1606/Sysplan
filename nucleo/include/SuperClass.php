...
class SuperClass extends MasterConexion {
    ...
    function whereWithLogicOperator($rules)
    {
        ...
        foreach ($rules as $key => $rule) {
            ...
            if (isset($rule[3])) { // Usar isset para verificar si el índice existe
                $query .= " {$rule[3]} ";
            }
        }
    ...
    function updateDB() {
        $resultado = false;
        reset($this->allvars); // Resetear el puntero del array antes de iterar
        foreach ($this->allvars as $key => $value) { // Iterar con foreach para mayor claridad
            if ($value !== NULL) { // Usar !== NULL para evitar problemas con 0 o cadenas vacías
                $query = "UPDATE " . $this->my_name . " SET " . $key . " = '" . utf8_encode($value) . "' WHERE id = '" . $this->allvars["id"] . "'";
    ...
    function __destruct() {
        // No llamar a parent::__destruct() aquí, ya que MasterConexion lo maneja.
    }
    ...
    public function insertar($tabla, $datos) {
        ...
        try {
            return parent::insert($sql, $datos);
        } catch (PDOException $e) {
            error_log("Error en SuperClass::insertar: " . $e->getMessage());
            return false;
        }
    }

    public function actualizar($tabla, $id_col, $id_val, $datos) {
        if (empty($datos)) {
            return false;
        }
        $set_clause = '';
        ...
        $sql = "UPDATE `$tabla` SET $set_clause WHERE `$id_col` = :id_val";
        $datos['id_val'] = $id_val;
        try {
            return parent::update($sql, $datos);
        } catch (PDOException $e) {
            error_log("Error en SuperClass::actualizar: " . $e->getMessage());
            return false;
        }
    }
}
