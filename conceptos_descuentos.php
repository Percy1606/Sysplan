...
/* Estilos personalizados para los botones de la tabla de descuentos */

/* Botón Editar (Azul) */
#tb_conceptos_descuentos .btn.btn-info {
    background-color: #00c0ef;
    border-color: #00c0ef;
    color: #fff;
}

#tb_conceptos_descuentos .btn.btn-info:hover {
    background-color: #00a3d8;
    border-color: #00a3d8;
}

/* Botón Eliminar (Rojo) */
#tb_conceptos_descuentos .btn.btn-danger {
    background-color: #dd4b39;
    border-color: #dd4b39;
    color: #fff;
}

#tb_conceptos_descuentos .btn.btn-danger:hover {
    background-color: #d73925;
    border-color: #d73925;
}

/* Estilos para la tabla de descuentos */
.table th, .table td {
...
#tb_conceptos_descuentos tr.odd td,
#tb_conceptos_descuentos tr:nth-child(odd) td {
    background-color: #f5f5f5 !important; /* Un gris muy claro, aún más sutil */
}

#tb_conceptos_descuentos tr.even td,
#tb_conceptos_descuentos tr:nth-child(even) td {
    background-color: #ffffff !important; /* Blanco */
}
</style>

<h1 class="header-title text-center">Gestión de Conceptos de Descuentos</h1>
<p class="text-center lbl">Aquí se gestionarán los conceptos de descuentos para la planilla.</p>
...
<div class="container-fluid mt-5">
        ...
        <div class="col-md-12">
            <div class="panel">
                <div class="panel-body">
                    <h2 class="section-title">Registrar/Editar Concepto de Descuento</h2>
    ...
    <div class="row mt-5">
        <div class="col-md-12">
            <div class="panel">
                <div class="panel-body">
                    <h2 class="section-title">Listado de Conceptos de Descuentos</h2>
                    <div class='contenedor-tabla'>
                        <table id='tb_conceptos_descuentos' class='display table table-bordered' cellspacing='0' width='100%'>
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Codigo</th>
...
<script>
    var table_descuentos; // Declarar la variable de la tabla fuera para que sea accesible

    $(document).ready(function() {
        // Inicializar DataTables una sola vez
        table_descuentos = $('#tb_conceptos_descuentos').DataTable({
            "language": {
                "url": "//cdn.datatables.net/plug-ins/1.10.25/i18n/Spanish.json"
            }
        });

        loadConceptosDescuentos();

        $('#form_concepto_descuento').submit(function(e) {
            e.preventDefault();
            saveConceptoDescuento();
        });
    });

    function loadConceptosDescuentos() {
        $.ajax({
            url: 'ws/conceptos_descuentos.php',
            type: 'POST',
            ...
            success: function(response) { // Cambiado a 'response' para mayor claridad
                if (response.status === 'success' && Array.isArray(response.data)) { // Asumiendo que el WS devuelve {status: 'success', data: [...]}
                    table_descuentos.clear(); // Limpiar la tabla existente
                    var data = [];
                    response.data.forEach(function(concepto) { // Iterar sobre response.data
                        data.push([
                            ...
                            `<button class="btn btn-info btn-sm" onclick="editConceptoDescuento(${concepto.id})"><i class="fa fa-edit"></i></button>
                             <button class="btn btn-danger btn-sm" onclick="deleteConceptoDescuento(${concepto.id})"><i class="fa fa-trash"></i></button>`
                        ]);
                    });
                    table_descuentos.rows.add(data).draw(); // Añadir nuevas filas y redibujar la tabla
                } else {
                    console.error("La respuesta de conceptos de descuentos no es un array o no tiene status success:", response);
                    table_descuentos.clear().draw(); // Limpiar la tabla si hay un error
                }
            },
            error: function(xhr, status, error) {
                console.error("Error al cargar conceptos de descuentos:", status, error, xhr.responseText);
                table_descuentos.clear().draw(); // Limpiar la tabla en caso de error
            }
        });
    }

    function saveConceptoDescuento() {
        var formData = $('#form_concepto_descuento').serializeArray();
    ...
    function editConceptoDescuento(id) {
        $.ajax({
            url: 'ws/conceptos_descuentos.php',
            type: 'POST',
            ...
            success: function(response) {
                if (response.status === 'success' && response.data) {
                    ...
                    console.error("Error al obtener concepto de descuento para edición:", response);
                    alert("Error al cargar concepto de descuento para edición.");
                }
            },
            error: function(xhr, status, error) {
                console.error("Error en la solicitud AJAX para editar concepto de descuento:", status, error, xhr.responseText);
                alert("Error al cargar concepto de descuento para edición.");
            }
        });
    }

    function deleteConceptoDescuento(id) {
        if (confirm('¿Está seguro de eliminar este concepto de descuento?')) {
            $.ajax({
                url: 'ws/conceptos_descuentos.php',
            type: 'POST',
